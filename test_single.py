...
flags.DEFINE_integer('height', 128, 'Input height')
flags.DEFINE_integer('width', 128, 'Input width')
...
def test(image_list):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for i, image_path in enumerate(image_list):
            ...
            save_img(img, os.path.join(FLAGS.resdir, 'img'), img_name)
            vis_semseg(_pred, cmap, os.path.join(FLAGS.resdir, 'pred'), img_name)
            message = 'test:{}'.format(FLAGS.resdir + '/' + img_name)
            print(message)
        coord.request_stop()
        coord.join(threads)
if __name__ == '__main__':
    i='rec_1_0_20151031DandongRapideyeprjclip.png'
    image_list=[os.path.join(FLAGS.indir,i)]
    print(image_list)
    # image_list = [os.path.join(FLAGS.indir, i) for i in os.listdir(FLAGS.indir)]
    test(image_list)